...
def model1(df1,store,dept,look_back=4):
	...
	plt.plot(scaler.inverse_transform(dataset[0:train_size,:]),label='Training_data')
	...
	plt.plot(train_preds_plt,label='Train_predictions')
	plt.plot(test_preds_plt,label='Future Predictions')
	plt.plot(list1.reshape(-1,1),'ro',label="Holidays")
	#plt.show()
	plt.legend(loc='upper left')
	plt.xlabel("Weeks")
	plt.ylabel('Weekly Sales')
	str1='./image1/image'+str(store)+str(dept)+".png"
	plt.savefig(str1)
	plt.close()
